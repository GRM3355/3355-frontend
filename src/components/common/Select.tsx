  ...
  return (
    <div className='relative'>
      ...
      <div onClick={(e) => handleShow(e, !show)}
        ...
        <span className="label5-r text-primary">{items.find(item => item.key === selected)?.label ?? items[0].label}</span>
        {show ? (<ChevronUp size={20} />) : (<ChevronDown size={20} />)}
      </div>
      {/* 선택지 */}
      {show &&
        <div className='absolute top-8 right-0 flex flex-col w-max bg-surface-bg-page sheet rounded-3 p-[3px]
        label5-r'>
          {items.map(item => (
            <span key={item.key} className={`${selected === item.key ? 'text-text-brand' : 'text-text-primary'} p-3 rounded-2`}
