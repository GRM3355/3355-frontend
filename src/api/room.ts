...
//방떠나기
export const leaveRoom = async ({ roomId, token, }: { roomId: string; token: string; }) => {
  console.log('방 join 요청:', { roomId });
  console.log('URL:', `/api/v1/chat-rooms/${roomId}/leave`);
...
export const getRoomsByUserId = async (token: string) => {
  const res = await apiClient.get('/api/v1/chat-rooms/my-rooms', {
    ...
    params: { page: 1, pageSize: 100, order: "ACTIVE_DESC" }, // 필요하면 고정값
  });
  return res.data.data;
};
